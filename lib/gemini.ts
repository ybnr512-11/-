...
${FACT_RULES}
항상 한국어로 답하세요.
장소·업체·맛집·카페를 추천하거나 직접 방문을 안내할 때만, 답변 말미에 Google 지도에서 영업시간·리뷰 확인을 짧게 안내하세요.
교통·행사·일반 정보·잡담 등 장소 방문과 무관한 답변에는 Google 지도·방문·영업시간·리뷰 확인 문구를 넣지 마세요.`;

const MAP_REMINDER_PATTERNS = [
  /방문\s*전\s*Google\s*지도[^.\n]*(?:확인|체크)[^.。\n]*[.。\s]*/gi,
  /Google\s*지도에서\s*(?:최신\s*)?(?:영업시간|리뷰)[^.。\n]*(?:확인|체크)[^.。\n]*[.。\s]*/gi,
  /Google\s*지도에서\s*최신\s*정보[^.。\n]*(?:확인|체크)[^.。\n]*[.。\s]*/gi,
];

function sanitizeChatReply(reply: string, placeRelated: boolean) {
  if (placeRelated) return reply.trim();
  let text = reply;
  for (const pattern of MAP_REMINDER_PATTERNS) {
...
export async function chatWithGemini(
  ...
  const reply = await withModelFallback((modelName) =>
    generateWithGoogleSearch(
      modelName,
      `${ASSISTANT_PERSONA}\nGoogle 검색 도구로 최신 정보를 확인한 뒤 답변하세요. 장소 추천·방문 안내가 아닌 질문에는 Google 지도·방문 관련 문구를 넣지 마세요.`,
      contents
    )
  );

  return { reply: sanitizeChatReply(reply, false), sources: ["Google 검색"], placeLinks: [] };
}
